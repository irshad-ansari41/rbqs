...
function admin_ctm_project_job_order_add_page() {
    ...
    if (!empty($postdata)) {
        ...
        $data = ['client_id' => $postdata['client_id'], 'quotation_id' => $postdata['quotation_id'], 'qtn_type' => $qtn_type, 'contact_no' => $postdata['contact_no'],
            ...
            'status' => $postdata['status'], 'created_by' => $current_user->ID, 'created_at' => $date, 'updated_at' => $date];
    ...
    <div class="wrap">
        ...
        <h1 class="wp-heading-inline">CREATE PROJECT JOB ORDER</h1>
        <br/>
        <br/>
            ...
            <div class="inside">
                <?php if (!empty($msg)) { ?>
                    ...
                    <div class="alert alert-success alert-dismissible">
                        ...
                        <strong>Success!</strong> Your Project Job Order has been saved successfully.
                    </div>
                <?php } ?>
                <form id="new-quotation-form" method="post" action="">               
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-3">
                            ...
                            <input type="text" name="quotation_id" id="quotation_id"  placeholder="Quotation No" required >
                        </div>

                        <div class="col-sm-3">
                            <label>Requested By:<span class="text-red">*</span></label><br/>
                            <input type="text" name="requested_by"  placeholder="Requested By" value="<?= $sales_person->display_name ?>" required>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12">
                            <label>Address:<span class="text-red">*</span></label><br/>
                            <textarea type="text" name="address" id="client-address"  placeholder="Address" rows="2" required></textarea>
                        </div>
                    </div>
                            ...
                            <table id="tbl-items" border="1" class="table-striped text-center boder-collapse" style="width: 100%;">
                                ...
                                <tr id="tbl-row-1" valign=top>
                                    ...
                                    <td>
                                        ...
                                        <textarea name="items[1][action]" id="input-action-1" rows="5" placeholder="Action to be Taken" ></textarea>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="date" name="items[1][start_date]" id="input-start-date-1" required>
                                        <br/><?= get_schedule_from('items[1][start_time]'); ?>
                                    </td>

                                    <td>
                                        <input type="date" name="items[1][end_date]" id="input-end-date-1"  required >
                                        <br/><?= get_schedule_to('items[1][end_time]'); ?>
                                    </td>
                                </tr>
    ...
    required ><?= $options ?></select></td>
                            ...
                            <td>
                                <input id="item-image-${rowCount}" data-input="items[${rowCount}][image]" data-output='output-image-${rowCount}' class="item-image button-primary" 
                                       type="button" value="Add Image" /><br/>
                                <output id="output-image-${rowCount}"></output>
                            </td>
                            ...
                            <td><input type="date" name="items[${rowCount}][end_date]" id="input-end-date-${rowCount}" required><br/>
    <?= get_schedule_from('items[${rowCount}][end_time]'); ?></td>
                        </tr>`;

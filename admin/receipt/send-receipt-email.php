...
function make_model_send_receipt_email($receipt) {
    global $current_user;
    $content_id = 'content';
    ...
    $subject = strtoupper("ROCHE BOBOIS RECEIPT {$receipt->id} {$client->name} {$qtn}");
    ...
    $content = "<p style='font-size:14px;'>Dear {$client->name},<p>"
            . "<p style='font-size:14px;'>Please see attached copy of Receipt <b>{$receipt->id}</b> as payment towards QTN <b>{$qtn}</b> </p>"

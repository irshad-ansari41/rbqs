...
function admin_ctm_bank_deposit_edit_page() {
    ...
    if (!empty($postdata['action'])) {
        ...
        $data = ['payment_status' => $postdata['payment_status'], 'change_amount' => $postdata['change_amount'], 'charges' => $postdata['charges'], 'hold_cash' => $postdata['hold_cash'], 'net_deposit' => $postdata['net_deposit'], 'bank_status' => $postdata['bank_status'], 'verify_date' => $postdata['verify_date'], 'updated_by' => $current_user->ID, 'updated_at' => $date];

        $wpdb->update("{$wpdb->prefix}ctm_bank_deposits", $data, ['id' => $id], wpdb_data_format($data), ['%d']);


        $receipt = get_receipt($postdata['receipt_id']);
        make_credit_transaction($id, $receipt->payment_date, $postdata['verify_date'], $receipt->received_from, $receipt->id, $postdata['net_deposit']);
    ...
    <div class="wrap">
        ...
        <h1 class="wp-heading-inline">Edit Bank Deposit</h1>
        <br/><br/>
                                    ...
                                    <table class="form-table" style="width:100%">
                                        ...
                                        <tr>
                                            <td><label>Receipt No:<span class="text-red">*</span></label>
                                                <input type=text name='receipt_id' readonly value="<?= $receipt->id ?>" >
                                            </td>
                                        ...
                                        <tr>
                                            <td><label>Payment Status:<span class="text-red">*</span></label><br/>
                                                <label class="font-weight-light">
                                                    <input type="radio" name="payment_status" value="Full" <?= $bank_deposit->payment_status == 'Full' ? 'checked' : '' ?> required />Full
                                                </label>&nbsp;&nbsp;
                                                <label class="font-weight-light">
                                                    <input type="radio" name="payment_status" value="Advance" <?= $bank_deposit->payment_status == 'Advance' ? 'checked' : '' ?> required />Advance</label>&nbsp;&nbsp;
                                                <label class="font-weight-light">
                                                    <input type="radio" name="payment_status" value="Balance" <?= $bank_deposit->payment_status == 'Balance' ? 'checked' : '' ?> required />Balance</label>&nbsp;&nbsp;
                                            </td>
                                            <td><label>Change:</label>
                                                <input type=number name="change_amount" id="change" min="0" step="0.01" max="<?= $receipt->paid_amount ?>"  placeholder="Change" 
                                                       value="<?= $bank_deposit->change_amount ?>">
                                            </td>
                                            <td><label>Charges:</label>
                                                <input type=number name="charges" id="charges" min="0" step="0.01" max="<?= $receipt->paid_amount ?>"  placeholder="Charges" 
                                                       value="<?= $bank_deposit->charges ?>">
                                            </td>
                                            <td><label>Cash on Hold:</label>
                                                <input type=number name="hold_cash"  id="hold_cash" min="0" step="0.01" max="<?= $receipt->paid_amount ?>" placeholder="Cash on Hold" 
                                                       value="<?= $bank_deposit->hold_cash ?>">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><label>Net Deposit:</label>
                                                <input type=number name="net_deposit"  id="net_deposit" min="0" step="0.01"  placeholder="Net Deposit" 
                                                       value="<?= $bank_deposit->net_deposit ?>" required>
                                            </td>
                                            <td><label>Bank Verification Status:</label><br/>
                                                <label class="font-weight-light">
                                                    <input type="radio" name="bank_status" value="Dummy" <?= $bank_deposit->bank_status == 'Dummy' ? 'checked' : '' ?> required />Dummy</label>
                                                &nbsp;&nbsp;
                                                <label class="font-weight-light">
                                                    <input type="radio" name="bank_status" value="Verified" <?= $bank_deposit->bank_status == 'Verified' ? 'checked' : '' ?> required />Verified</label>
                                                &nbsp;&nbsp;

                                            </td>
                                            <td><label>Bank Verification Date:</label><br/>
                                                <input type=date name="verify_date" value="<?= $bank_deposit->verify_date ?>">
                                            </td>
                                            <td/>
                                        </tr>
    ...
    <script>
        ...
        jQuery(document).ready(() => {
            ...
            jQuery('#net_deposit').on('input blur', () => {
                var receipt_amount = parseFloat(<?= $receipt->paid_amount ?>);

...
class CTM_Tax_Credit_Note_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $sql = "SELECT t1.id,t1.tax_invoice_id,t1.updated_by,t1.status, t1.cn_type, t1.updated_at, t1.quotation_id, t1.meta_ids,t2.type,t2.scope,t2.vat,t2.client_id FROM {$wpdb->prefix}ctm_tax_credit_note t1 LEFT JOIN  {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id  $where LIMIT $start, 10";
        ...
        if ($wpdb->num_rows > 0) {
            foreach ($rs as $value) {
                ...
                $listArr[$i]['ti_id'] = $value->tax_invoice_id;
                ...
                $listArr[$i]['cn_type'] = $value->cn_type == 'GCN' ? 'With Goods' : 'Without Goods';
                ...
                $view = "<a href='" . admin_url() . "admin.php?page=tax-credit-note-view&id={$value->id}' class='btn-view' title='View' >"
        ...
        $count = $wpdb->get_var("SELECT count(t1.id) FROM {$wpdb->prefix}ctm_tax_credit_note t1 LEFT JOIN  {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id $where"); //count($rows);
...
function admin_ctm_tax_credit_note_page() {
    global $wpdb;
    ...
    if ($id && $action == 'delete') {
        $wpdb->query("DELETE FROM {$wpdb->prefix}ctm_tax_credit_note WHERE id={$id}");
    ...
    <div class="wrap">
        ...
        <h1 class="wp-heading-inline">Tax Credit Note List</h1>
        <br/><br/>
        <?php if (!empty($getdata['msg'])) { ?>
            <br/>
            <div class="alert alert-success alert-dismissible">
                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                Tax Credit Note has been created successfully
            </div>
        <?php } ?>
                            ...
                            <div class="inside">
                                <div id="page-inner-content1">
                                    ...
                                    <form id="deletes-filter" method="get">
                                        ...
                                        $testListTable = new CTM_Tax_Credit_Note_List_Table();

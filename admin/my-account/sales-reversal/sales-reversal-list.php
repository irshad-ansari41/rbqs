...
class CTM_Sales_Reversal_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $sql = "SELECT t1.id,t1.updated_by,t1.updated_at, t1.quotation_id, t1.meta_ids,t1.status,t2.type,t2.scope,t2.vat,t2.client_id,t2.sales_person FROM {$wpdb->prefix}ctm_sales_reversal t1 LEFT JOIN  {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id  $where LIMIT $start, 10";
        ...
        if ($wpdb->num_rows > 0) {
            foreach ($rs as $value) {
                ...
                $view = !empty($value->meta_ids) ? "<a href='" . admin_url() . "admin.php?page=sales-reversal-view&id={$value->id}' class='btn-view' title='View' ><img alt='' src='{$asset_url}view.png'></a>&nbsp;&nbsp;&nbsp;" : '';
                $edit = "<a href='admin.php?page={$page}-edit&id={$value->id}&action=edit'  class=btn-edit><img alt='' src='{$asset_url}edit.png' title='Edit'></a>&nbsp;&nbsp;&nbsp;";
                ...
                $listArr[$i]['action'] = $view . $edit . (has_this_role('accounts') ? $delete : '');
        ...
        $count = $wpdb->get_var("SELECT count(t1.id) FROM {$wpdb->prefix}ctm_sales_reversal t1 LEFT JOIN  {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id $where"); //count($rows);
...
function admin_ctm_sales_reversal_page() {
    global $wpdb;
    ...
    if ($action == 'create') {
        ...
        $wpdb->query("DELETE FROM {$wpdb->prefix}ctm_sales_reversal WHERE id={$id}");
        ...
        <div class="wrap">
            ...
            <h1 class="wp-heading-inline">Reversal of confirmed QTN</h1>
                    ...
                    <div id="postbox-container" class="postbox-container">
                        <div id="normal-sortables" class="meta-box-sortables ui-sortable">
                            <div id="welcome-to-aquila" class="postbox"><br/>
                                ...
                                <div class="inside">
                                    <form id="add-new-category-form" class="hide" method="post">
                                        <input type="hidden" name="page"  value="<?= $getdata['page'] ?>" >
                                        <table class="form-table">
                                            <tr>
                                                <td><label>QTN No:<span class="text-red">*</span></label><br/>
                                                    <input type="number" name="qid" id="qid" class="search-input" placeholder="QTN" required><br/>
                                                    <span id="msg-1" class="text-red"></span>
                                                </td>
                                                <td><label>Revised QTN:</label><br/>
                                                    <input type="text" class="search-input" id="revised_no" placeholder="Revised QTN" ><br/>
                                                </td>
                                                <td><label>Receipt No:<span class="text-red">*</span></label><br/>
                                                    <input type="number" name="receipt_no" id="receipt_no" class="search-input" placeholder="Receipt No" required><br/>
                                                    <span id="msg-2" class="text-red"></span>
                                                </td>
                                                <td><br/><br/><button type="submit"  name="action" value="create" class="button-primary">Create</button>
                                                <td><br/><br/><a href="?page=<?= $getdata['page'] ?>"  class="button-secondary" >Cancel</a></td>
                                            </tr>
                                        </table>
                                        <br/>
                                    </form>
                                    ...
                                    <div id="page-inner-content">
                                        ...
                                        <form id="deletes-filter" method="get">
                                            ...
                                            $testListTable = new CTM_Sales_Reversal_List_Table();
        ...
        <script>
            jQuery(document).ready(() => {
                ...
                jQuery('#qid,#revised_no,#receipt_no').on('blur', function () {
                    var qid = jQuery('#qid').val();
                    var revised_no = jQuery('#revised_no').val();
                    var receipt_no = jQuery('#receipt_no').val();
                    jQuery.ajax({
                        url: "<?= get_template_directory_uri() ?>/ajax/check-sales-reversal.php",
                        dataType: 'json',
                        data: {qid: qid, revised_no: revised_no, receipt_no: receipt_no},
                        success: function (data) {
                            if (data.qid === false) {
                                jQuery('#msg-1').text('Invalid quotation No');
                                jQuery('#qid').val('');
                            } else if (data.receipt_no === false) {
                                jQuery('#msg-2').text('Invalid receipt No');
                                jQuery('#qid').val('');
                            } else {
                                jQuery('#msg-1,#msg-2').text('');
                                jQuery('#qid').val(data.qid);
                                jQuery('#revised_no').val(data.revised_no);
                                jQuery('#receipt_no').val(data.receipt_no);
                            }
                        }
                    });
                });

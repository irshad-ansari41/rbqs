...
class CTM_Tax_Invoice_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $sql = "SELECT t1.id,t1.updated_by,t1.updated_at, t1.quotation_id, t1.meta_ids,t2.type,t2.scope,t2.vat,t2.client_id FROM {$wpdb->prefix}ctm_tax_invoice t1 LEFT JOIN  {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id  $where LIMIT $start, 10";
        ...
        if ($wpdb->num_rows > 0) {
            foreach ($rs as $value) {
                $tax_credit_note = check_tax_credit_note($value->id);
                ...
                $view = "<a href='" . admin_url() . "admin.php?page=tax-invoice-view&id={$value->id}' class='btn-view' title='View' ><img alt='' src='{$asset_url}view.png'></a>&nbsp;&nbsp;&nbsp;";
                $create_view = $tax_credit_note ? "<a href='" . admin_url() . "admin.php?page=tax-credit-note-view&id={$tax_credit_note->id}' class=' btn-dark btn-sm' title='View' >View Tax Credit Note </a>&nbsp;&nbsp;&nbsp;" : '';
                ...
                $listArr[$i]['action'] = $view . $create_view . (has_this_role('accounts') ? $delete : '');
                $i++;
            }
        }
        $data = $listArr;
        ...
        $count = $wpdb->get_var("SELECT count(t1.id) FROM {$wpdb->prefix}ctm_tax_invoice t1 LEFT JOIN  {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id $where"); //count($rows);
...
function admin_ctm_tax_invoice_page() {
    global $wpdb;
    ...
    if ($id && $action == 'delete') {
        $wpdb->query("DELETE FROM {$wpdb->prefix}ctm_tax_invoice WHERE id={$id}");
        $wpdb->query("DELETE FROM {$wpdb->prefix}ctm_tax_credit_note WHERE tax_invoice_id='$id'");
        wp_redirect("admin.php?page={$getdata['page']}&msg=delete");
        exit();
    }
    $pjo_id = !empty($getdata['pjo_id']) ? $getdata['pjo_id'] : '';
    if ($pjo_id && $action == 'create') {
        create_tax_invoice_pjo($pjo_id);
    ...
    <div class="wrap">
        ...
        <h1 class="wp-heading-inline">Tax Invoice List</h1>
        <br/><br/>
                            ...
                            <div class="inside">
                                <div id="page-inner-content1">
                                    ...
                                    <form id="deletes-filter" method="get">
                                        ...
                                        $testListTable = new CTM_Tax_Invoice_List_Table();

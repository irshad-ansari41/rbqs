...
function admin_ctm_stf_packaging_label_page() {
    global $wpdb;
    $getdata = filter_input_array(INPUT_GET);
    $postdata = filter_input_array(INPUT_POST);
    $date = date('d-m-Y');
    $page = !empty($getdata['page']) ? $getdata['page'] : '';
    $stf_id = !empty($getdata['stf_id']) ? $getdata['stf_id'] : '';

    if (!empty($postdata)) {
        update_option("packing_label_{$stf_id}", $postdata['pack']);
    }


    $po_meta_items = [];
    if (!empty($stf_id)) {
        $stf_items = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}ctm_stock_transfer_meta WHERE stf_id='{$stf_id}'");
        foreach ($stf_items as $value) {
            $po_meta_items[] = get_po_meta_data($value->po_meta_id);
        }
        $packs = get_option("packing_label_{$stf_id}");
    }
    ?>
    ...
    <div class="wrap">
        ...
        <a href = 'admin.php?page=stock-trasfer' class='page-title-action'>Back</a>&nbsp;
        <br/><br/>
                            ...
                            <div id='page-inner-content' class='postbox'><br/>
                                <div class='inside' style='max-width:100%;margin:auto'>
                                    ...
                                    foreach ($po_meta_items as $value) {
                                        ...
                                        $st_qtn = $value->client_id == FLAGSHIP_ID ? "STF NO: {$stf_id}" : "QTN NO: {$qtn}";
                                        $table .= "<tr><td style='font-size:80px;text-align:center;text-transform:uppercase'>" . get_client($value->client_id, 'name') . "</td></tr>"
                                                . "<tr><td style='font-size:80px;text-align:center;'>$st_qtn</td></tr>"
                                    ...
                                    $html .= $table;
                                    echo $html;
                                    $pdf_file = make_pdf_file_name("PACKAGE_LABEL_{$stf_id}.pdf")['path'];
                                    ?>
                                </div>
                            </div>
                            <div class="row btn-bottom">
                                <div class="col-sm-6 text-left">
                                    <a href = 'admin.php?page=stock-trasfer' class='btn btn-secondary btn-sm text-white'>Back</a>&nbsp;
                                    <?php

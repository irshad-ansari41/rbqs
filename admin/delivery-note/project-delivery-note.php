...
function admin_ctm_project_delivery_note_page() {
    ...
    if (empty($dn_id)) {
        create_project_delivery_note($qid);
    ...
    if (!empty($postdata['status']) && $postdata['status'] == 'DELIVERED') {
        ...
        $wpdb->query("UPDATE {$wpdb->prefix}ctm_quotations SET status='{$postdata['status']}', updated_at='{$date}' WHERE id='{$quotation_id}'");
        create_tax_invoice_project($quotation_id);
    }
    ...
    $html = "
                ...
                <div id='welcome-to-aquila' class='postbox'>
                  ...
                  <td class='text-center' colspan=2><b>SUP</b></td>
    ...
    foreach ($dn_meta as $value) {
        ...
        $tr = "<tr>"
                . "<td class='text-center' colspan=2>{$value->sup_code}</td>"
    ...
    $html .= "<tr class='text-right font-weight-bold'>
                  ...
                  <tr><td colspan=2 rowspan=2><b>RECEIVER'S SIGN</td><td rowspan=3 colspan=3></td><td><b>DELIVERY&nbsp;DATE</b></td><td><b>" . rb_date($dn->delivery_date) . "</b></td></tr>

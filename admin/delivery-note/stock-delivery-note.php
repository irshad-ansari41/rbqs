...
function admin_ctm_stock_delivery_note_page() {

    global $wpdb, $current_user;
    $getdata = filter_input_array(INPUT_GET);
    $postdata = filter_input_array(INPUT_POST);
    $date = current_time('mysql');
    $qid = !empty($getdata['qid']) ? $getdata['qid'] : 0;
    $dn_id = !empty($getdata['dn_id']) ? $getdata['dn_id'] : 0;
    if (empty($dn_id) && !empty($qid)) {
    ...
    if (!empty($postdata['delivery_note'])) {
        $sql = "UPDATE {$wpdb->prefix}ctm_quotation_dn SET address='{$postdata['address']}', receiver_name='{$postdata['receiver_name']}', delivered_by='{$postdata['delivered_by']}', delivery_date='{$postdata['delivery_date']}', delivery_time_from='{$postdata['delivery_time_from']}', delivery_time_to='{$postdata['delivery_time_to']}', updated_by='{$current_user->ID}', updated_at='{$date}' WHERE id='{$dn_id}'";
        $wpdb->query($sql);
        update_dn_meta_location($postdata);
    }

    if (!empty($postdata['status']) && $postdata['status'] == 'DELIVERED') {
        $dn = $wpdb->get_row("SELECT quotation_id,client_id FROM {$wpdb->prefix}ctm_quotation_dn WHERE id='{$dn_id}'");
        $wpdb->query("UPDATE {$wpdb->prefix}ctm_quotation_dn SET status='{$postdata['status']}', updated_at='{$date}' WHERE id='{$dn_id}'");
        $wpdb->query("UPDATE {$wpdb->prefix}ctm_quotations SET status='{$postdata['status']}', updated_at='{$date}' WHERE id='{$dn->quotation_id}'");
        stock_qtn_update_stock_inventroy($dn->quotation_id, $dn->client_id, 'DELIVERED');
        create_tax_invoice_stock($dn->quotation_id);
    }
    ...
    if (!empty($postdata['tax_invoice'])) {
        ...
        create_tax_invoice_stock($quotation_id);
    }
    ...
    $total_amount = get_qtn_total_amount($dn->quotation_id);
    $paid_amount = get_qtn_paid_amount($dn->quotation_id);
    $balance_amount = $total_amount - $paid_amount;
    if (!has_role_super_and_admin() && $total_amount != $paid_amount) {
        echo $html = "<br/><br/><br/>
                <div id='welcome-to-aquila' class='postbox'>
                    <div class='inside' style='max-width:800px;margin:auto;text-align:center'><br/>
                    <strong style='color:red;font-size:30px'>Outstanding Balance Amount AED " . number_format($balance_amount, 2) . " to be settled.<br/>
                    Delivery Note cannot be generated</strong>
                    </div>
                </div>";
        exit();
    }
    ...
    $dn_meta = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}ctm_quotation_dn_meta where dn_id='{$dn->id}'");
    ...
    $html = "
                ...
                <div id='welcome-to-aquila' class='postbox'>
                  ...
                  <tr>
                  <td><b>Name:</b></td><td colspan='4'><b>{$client->name}</b></td><td class='text-center'><b>{$dn->id}</b></td><td class='text-center'><b>$updated_at</b></td>
                  </tr>
                  <tr>
                  <td rowspan=2><b>Address</b></td><td colspan='4'><b>" . ($dn->address ? $dn->address : $client->address) . "</b></td><td class='bg-blue text-center' ><b>Quotation #</b></td><td class='bg-blue text-center'><b>Destination</b></td>
                  </tr>
                  <tr>
                  <td colspan='4'><b>{$client->city}, {$client->country}</b></td><td class='text-center'><b>{$qtn}</b></td><td class='text-center'><b>" . get_location($quotation->city_id, 'city') . "</b></td>
                  </tr>
                  <tr>
                  <td><b>Phone</b></td><td colspan='4'><b>{$client->phone}</b></td><td  class='bg-blue text-center' rowspan='2' colspan=2 ><b>Roche Bobois<br/>TRN# 100383178900003</b></td>
                  </tr>
    ...
    $html .= "<tr class='text-right font-weight-bold'>
                  ...
                  <tr><td colspan=2><b>RECEIVER'S NAME</b></td><td colspan=3><b>{$dn->receiver_name}</b></td><td><b>DELIVERED&nbsp;BY</td><td width='150'><b>{$dn->delivered_by}</b></td></tr>
                  <tr><td colspan=2 rowspan=2><b>RECEIVER'S SIGN</td><td rowspan=2  colspan=3 ></td><td><b>DELIVERY&nbsp;DATE</b></td><td><b>" . rb_date($dn->delivery_date) . "</b></td></tr>
                  <tr><td><b>DELIVERY&nbsp;TIME</td><td><b>" . rb_time($dn->delivery_time_from, 'h:i a') . " " . rb_time($dn->delivery_time_to, 'h:i a') . "</b></td></tr>
                  </table>

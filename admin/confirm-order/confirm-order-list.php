...
class CTM_Quotation_Confirm_order_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $from_date = !empty($from_date) ? "t.created_at>='{$from_date} 00:00:01'" : 1;
        $to_date = !empty($to_date) ? "t.created_at<='{$to_date} 23:59:59'" : 1;
        $qtn_scope = !empty($qtn_scope) ? " t2.scope ='{$qtn_scope}'" : 1;
        $qtn_type = !empty($qtn_type) ? " t2.type ='{$qtn_type}'" : 1;
        $status = !empty($status) ? " t1.status ='{$status}'" : 1;

        $where = "WHERE $client_id AND $qid AND $from_date AND $to_date AND $qtn_scope AND $qtn_type AND $status";
        $sql = "SELECT t1.id,t1.quotation_id,t1.revised_no,t1.client_id, t1.updated_at,t1.status,t2.type,t2.vat,t2.scope FROM {$wpdb->prefix}ctm_quotation_co t1 LEFT JOIN {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id  $where ORDER BY t1.updated_at DESC LIMIT $start, 10";
        ...
        if ($wpdb->num_rows > 0) {
            foreach ($rs as $value) {
                ...
                $listArr[$i]['qtn'] = (!empty($quotation->revised_no) ? $quotation->revised_no : $quotation->id);
                $listArr[$i]['client_name'] = get_client($value->client_id, 'name');
                $listArr[$i]['type'] = $quotation->type;
                $listArr[$i]['scope'] = $quotation->scope;
                $listArr[$i]['vat'] = $quotation->vat == 'wovat' ? 'Zero VAT' : '';
                $listArr[$i]['status'] = $value->status == 'ORDERED' ? "<span class='badge badge-success'>ORDERED</span>" : "<span class='badge badge-primary'>Pending</span>";
                $listArr[$i]['updated_at'] = rb_datetime($value->updated_at);
                $listArr[$i]['action'] = "<a href='" . admin_url() . "admin.php?page=confirm-order-items&co_id={$value->id}' class='btn-edit' title='View'><img alt='' src='{$asset_url}view.png'></a><br/>";
                $i++;
            }
        }
        $data = $listArr;
        ...
        $count = $wpdb->get_var("SELECT count(t1.id) FROM {$wpdb->prefix}ctm_quotation_co t1 LEFT JOIN {$wpdb->prefix}ctm_quotations t2 ON t1.quotation_id=t2.id $where");
...
function admin_ctm_confirm_order_list_page() {
    global $wpdb;
    ...
    <style>#page-inner-content input[type=text], #page-inner-content input[type=search], #page-inner-content input[type=tel], #page-inner-content input[type=time], #page-inner-content input[type=url], #page-inner-content input[type=week], #page-inner-content input[type=password], #page-inner-content input[type=color], #page-inner-content input[type=date], #page-inner-content input[type=datetime], #page-inner-content input[type=datetime-local], #page-inner-content input[type=email], #page-inner-content input[type=month], #page-inner-content input[type=number], #page-inner-content select, #page-inner-content textarea { width: 100%; }
        ...
        .attachment-large{width:80px;height: auto;}
    </style>
    <div class="wrap">
        <span id="open-close-menu" title="Close & Open Side Menu" class="dashicons dashicons-editor-code"></span> 
        <h1 class="wp-heading-inline">Confirmed Order</h1>
        <br/><br/>
                            ...
                            <div class="inside">
                                <div id="page-inner-content1">
                                    ...
                                    <form id="deletes-filter" method="get">
                                        ...
                                        $testListTable = new CTM_Quotation_Confirm_order_List_Table();

...
function make_model_send_account_email($client_id) {
    global $current_user;
    $date = date('Y-m-d');
    $content_id = 'content';
    ...
    $subject = strtoupper("ROCHE BOBOIS ACCOUNT STATEMENT OF {$client->name}");
    $designation = get_user_meta($current_user->ID, 'designation', true);
    $filename = make_pdf_file_name("account_statement_{$client->name}_{$date}.pdf")['path'];

    $content = "<p style='font-size:14px;'>Dear {$client->name},<p>"
            . "<p style='font-size:14px;'>Please see the attached account statement.</p>"

...
function admin_ctm_ol_report_page() {
    global $wpdb;
    $getdata = filter_input_array(INPUT_GET);

    $from_date = $getdata['from_date'] ?? date('Y-m-01');
    $to_date = $getdata['to_date'] ?? date('Y-m-t');


    $whr_from_date = !empty($from_date) ? " created_at >='{$from_date} 00:00:01'" : " created_at=" . date('Y-m-01 00:00:01');
    $whr_to_date = !empty($to_date) ? " created_at <='{$to_date} 23:59:59'" : " created_at=" . date('Y-m-t 23:59:59');
    $results = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}ctm_quotation_po_meta WHERE cl_status=1 AND order_registry='ARRIVED' AND $whr_from_date AND $whr_to_date ");
    ?>
    <style>
    ...
    <div class="wrap">
        <h1 class="wp-heading-inline">Offloading List Report</h1>
        <br/>
        <form method="get">
                    ...
                    <div id="normal-sortables" class="meta-box-sortables ui-sortable">
                        <div id="page-inner-content" class="postbox">
                            ...
                            $table = "<table width='800'>
                                                ...
                                                <tr>
                                                    <td colspan='2' style='text-align:center'><br/><br/>
                                                        <h6><span style='font-size:22px;'>OFFLOADING LIST REPORT</span></h6><br/><br/>
                                                    </td>
                                                </tr>
                                            </table>
                            ...
                            $table .= "<tr class=bg-blue>
                                      <td colspan='9'><b>Date</b></td></td>
                                      </tr>
                                      <tr>
                                        <td class='text-center'><b>From Date</b></td>
                                        <td class='text-center' colspan=3><b>" . rb_date($from_date) . "</b></td>
                                        <td class='text-center'  colspan=2><b>To Date</b></td>
                                        <td class='text-center' colspan=3><b>" . rb_date($to_date) . "</b></td>
                                      </tr>
                                      ";


                            $table .= "<tr><td  colspan='9'><br/></td></tr>
                                      <tr class='text-center bg-blue'>
                                        <td class='text-center'><b>Supper Name</b></td>
                                        <td class='text-center'><b>Entry # </b></td>
                                        <td class='text-center'><b>Description</b></td>
                                        <td class='text-center'><b>QTY</b></td>
                                        <td class='text-center'><b>PKG</b></td>
                                        <td class='text-center'><b>CQUE</b></td>
                                        <td class='text-center;'><b>Contact Number</b></td>
                                        <td class='text-center;'><b>Container Name</b></td>
                            ...
                            foreach ($results as $value) {
                                ...
                                $client = get_client($quotation->client_id ?? 0);
                                $client_name = $client->name ?? '';
                                $client_phone = $client->phone ?? '';
                                $client_phone2 = $client->phone2 ?? '';
                                $quotation_meta = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}ctm_quotations_meta where quotation_id='{$value->quotation_id}' AND item_id='{$value->item_id}'");
                                ...
                                $table .= "<tr class='text-center'>
                                        ...
                                        <td  style='text-align:left'>" . nl2br($quotation_meta->item_desc ?? '') . "</td>
                                        <td class='text-center'>$value->quantity</td>
                                        <td class='text-center'>$value->cl_pkgs</td>
                                        <td class='text-center'>" . ($client_name . ' ' . $qtn) . "</td>
                                        <td class='text-center'>" . $client_phone . $client_phone2 . "</td>
                                        <td class='text-center'>" . $value->container_name . "</td>
                                        <td class='text-center'>" . rb_datetime($value->updated_at) . "</td>
                                      </tr>";
                            }

                            $table .= "<tr><td colspan='9'><br/></td></tr></table>";
                            ...
                            echo $html;
                            $pdf_file = make_pdf_file_name("offloading_list_report_{$from_date}_{$to_date}.pdf")['path'];
                            ?>
                        ...
                        <div class="row btn-bottom">
                            <div class="col-sm-12 text-center">
                                ...
                                ?>
                                <a href = '<?= export_excel_report($pdf_file, 'offloading_list_report', $table) ?>' class='btn btn-success text-white btn-sm '>Export as Excel</a>
                            </div>
                        </div>
                    </div>	
                </div>
